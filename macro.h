...
{
    GtkWidget *win;
    info *accel; //acc //gtk_window_add_accel_group
    info *sh;
    info *back;//prvodier     //back
    GtkWindowType type_win;//typewin
    info *title;            //gtk_window_set_title ()   //title
    gboolean resiasable;    //gtk_window_set_resizable()    //resia
    gboolean modal;         //gtk_window_set_modal ()   //modal
    int default_size[2];    //gtk_window_set_default_size ()    //dfs
    int request_size[2];    //gtk_widget_set_size_request()     //rqs
    info *icon;             //gtk_window_set_icon_from_file ()  //ico
    double opacity;         //gtk_widget_set_opacity()
    gboolean fullscreen;    //gtk_window_fullscreen ()  //fulls
    gboolean keep_below;    //gtk_window_set_keep_below()   //kepb
...
window *creer_initialiser_win()
{
    ...
    win->sh=creer_initialiser_info();
    win->accel=creer_initialiser_info();
    win->back=creer_initialiser_info();
    remplir_info(win->icon,"default");
    win->resiasable=TRUE;
    win->modal=FALSE;
    ...
    remplir_info(win->icon,
                 "C:\\Users\\USER\\Desktop\\GTK\\projet_xml\\google-docs.png");
    win->opacity=1;
...
window *remplir_window(FILE *f1)
{
    ...
    while (p)
    {
        ...
        else if(strcmp(p->attribut->info,"acc")==0)
            remplir_info(wn_info->accel,p->valeur->info);
        else if(strcmp(p->attribut->info,"sh")==0)
            remplir_info(wn_info->sh,p->valeur->info);
        p=p->suiv;
    }
...
void make_window(window *wn_info)
{
    ...
    if(wn_info->sh->Idelem!=-1)win2=window;
    wn_info->win=window;
}
...
typedef struct hdb
{
    ...
    info *icon;
    info *title;                //gtk_header_bar_set_title ()   //title
    info *sub_title;            //gtk_header_bar_set_subtitle ()    //subt
    gboolean close_button_show; //gtk_header_bar_set_show_close_button ()   //clbsh
    // menu:minimize,maximize,close
    info *decoration_layout;    //gtk_header_bar_set_decoration_layout ()   //declay
...
header_bar *remplir_header_bar(FILE *f1)
{
    ...
    while (p)
    {
        ...
        else if(strcmp(p->attribut->info,"icon")==0)
            remplir_info(hdd->icon,p->valeur->info);
        p=p->suiv;
    }
...
void make_headerbar(header_bar *header_bar_info)
{
    ...
    if(header_bar_info->icon->Idelem!=-1)
    {
        GdkPixbuf *pixbuf;
        GdkPixbuf *scaled_pixbuf;
        pixbuf = gdk_pixbuf_new_from_file("C:\\Users\\USER\\Desktop\\GTK\\exam_tp\\icon.png", NULL);
        scaled_pixbuf = gdk_pixbuf_scale_simple(pixbuf, 20, 20, GDK_INTERP_BILINEAR);
        im = gtk_image_new_from_pixbuf(scaled_pixbuf);
        gtk_header_bar_pack_start(GTK_HEADER_BAR(hd_bar),im);
    }
...
{
    GtkWidget *button;
    GtkAlign alignh;//alignh
    GtkAlign alignv;//alignv
    int rqs[2];//req
    info *back;//provider   //back
    info *signal;//gtk_signal_connect //signal
    info *label;                    //gtk_button_new_with_label ()  //lab
    info *mnemonic;                 //gtk_button_new_with_mnemonic ()   //mnemonic
    info *button_from_icon_name;    //gtk_button_new_from_icon_name()   //bticf
    GtkIconSize icon_siz;   //ics
    GtkReliefStyle relief_type;     //gtk_button_set_relief();  //rlt //bordure button
    gboolean use_underline;         //gtk_button_set_use_underline ()   //usun  //surligner le text du button
    GtkPositionType image_position; //gtk_button_set_image_position ()  //impos
    //gtk_button_set_image()
    //gtk_button_set_label()
    //gtk_widget_set_tooltip_text //to set a tooltop text for button
}button;
...
button *remplir_button(FILE *f1)
{
    ...
    while (p)
    {
        ...
        {
            remplir_info(btns->button_from_icon_name,p->valeur->info);
            btns->label->Idelem=-1;
        ...
        }else if(strcmp(p->attribut->info,"req")==0)
        {
            sscanf(p->valeur->info,"%d|%d",&val1,&val2);
            btns->rqs[0]=val1;
            btns->rqs[1]=val2;
        }else if(strcmp(p->attribut->info,"alignh")==0)
        {
            if(strcmp(p->valeur->info,"GTK_ALIGN_START")==0)btns->alignh=GTK_ALIGN_START;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_CENTER")==0)btns->alignh=GTK_ALIGN_CENTER;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_END")==0)btns->alignh=GTK_ALIGN_END;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_FILL")==0)btns->alignh=GTK_ALIGN_FILL;
        }else if(strcmp(p->attribut->info,"alignv")==0)
        {
            if(strcmp(p->valeur->info,"GTK_ALIGN_START")==0)btns->alignv=GTK_ALIGN_START;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_CENTER")==0)btns->alignv=GTK_ALIGN_CENTER;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_END")==0)btns->alignv=GTK_ALIGN_END;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_FILL")==0)btns->alignv=GTK_ALIGN_FILL;
        }
        p=p->suiv;
    }
    return (button*)btns;
...
void func2(GtkWidget *button,gpointer data)
{
    gtk_widget_show_all(im1);
}
...
void make_button(button *button_info)
{
    ...
    if(button_info->signal->Idelem!=-1)
    {
        ...
        if(strcmp(button_info->signal->info,"func1")==0)g_signal_connect(btn,"clicked",G_CALLBACK(func1),NULL);
        ...
        else if(strcmp(button_info->signal->info,"hiii")==0)g_signal_connect(btn,"clicked",G_CALLBACK(func2),im1);
    }
    if(button_info->rqs[0]!=-1)
    {
        gtk_widget_set_size_request(btn,button_info->rqs[0],button_info->rqs[1]);
        gtk_widget_set_halign(btn,button_info->alignh);
        gtk_widget_set_valign(btn,button_info->alignv);
    }
...
typedef struct lb{
    GtkWidget *label;
    info *label_name;//gtk_label_new ()     //labna
    info *label_with_mnemonic;//gtk_label_new_with_mnemonic()   //labmn
    info *markup_text;//gtk_label_set_markup ()     //labmar  // "<span foreground=\"red\" font_desc=\"20\">Hello, world!</span>";
    info *markup_text_with_mnemonic;//gtk_label_set_markup_with_mnemonic () //labmarm
    info *pattern;//gtk_label_set_pattern ()    //patt
    GtkJustification justify;//gtk_label_set_justify ()     //just
    gdouble xalign;//gtk_label_set_xalign ()    //xlig
    gdouble yalign;//gtk_label_set_yalign ()    //ylig
    PangoEllipsizeMode ellipsis;//gtk_label_set_ellipsize ()    //ellip
    gboolean selectable;//gtk_label_set_selectable()    //selec
}label;
...
typedef struct im{
    GtkWidget *image;
    int reqsize[2];//gtk_request_size() //rqs
    info *file_name;//gtk_image_new_from_file ()    //nmfil
    info *signal;//g_signal_connect()   //signal
    info *animation_file;//gtk_image_new_from_animation () et gdk_pixbuf_animation_new_from_file()  //anmf
    info *icon_name;//gtk_image_new_from_icon_name ()   //icona
    GtkIconSize icon_siz;   //icons
    int pixel_size;//gtk_image_set_pixel_size ()    //pix
}image;
...
typedef struct radbtn
{
    ...
    info *label;//gtk_radio_button_new_with_label()     //label
    info *label_with_mnemonic;//gtk_radio_button_new_with_mnemonic ()   //labmne
    gboolean main_rdb;//pour savoir que le radio button est le pere     //mainrdb
    gboolean mode;//gtk_toggle_button_set_mode()    //mode
...
typedef struct chb
{
    ...
    info *label;//gtk_check_button_new_with_label ()    //label
    info *label_with_mnemonic;//gtk_check_button_new_with_mnemonic ()   //labmne
    gboolean mode;//gtk_toggle_button_set_mode()    //mode
}check_button;
...
//    g_signal_connect(chbt,"clicked",G_CALLBACK(raddd),check_button_info->label->info);
...
typedef struct tgb{
    ...
    info *label;//gtk_toggle_button_new_with_label ()   //label
    info *label_with_mnemonic;//gtk_toggle_button_new_with_mnemonic ()  //labmne
    gboolean mode;//gtk_toggle_button_set_mode()    //mode
    gboolean inconsitant;//gtk_toggle_button_set_inconsistent()
    //gtk_toggle_button_get_active(); //True is button is toggled or False if not
}toggle_button;
...
typedef struct linb
{
    ...
    info *label;//gtk_link_button_new_with_label ()     //label
    info *url;//gtk_link_button_set_uri ()  //url
    gboolean visited;//gtk_link_button_set_visited ()   //vise
}link_button;
...
typedef struct mnb{
    ...
    GtkArrowType direction;//gtk_menu_button_set_direction () //dire
...
typedef struct mnit
{
    info *label;//gtk_menu_item_new_with_label ()   //label
    info *mnemonic;//gtk_menu_item_new_with_mnemonic ()     //mnemo
    guint shor;//gtk_widget_add_accelerator     //short
    info *signal;//signal
    info *icon;//icon
    int cpt;
    struct mnit *but;
...
menu_item *creer_intialiser_menu_item()
{
    ...
    mnit->signal=creer_initialiser_info();
...
void on_menuitem_clicked(G_GNUC_UNUSED GMenuItem *menuitem,gpointer data)
{
    printf("item clicked  !!.\n");
}
...
menu_item *remplir_menu_item(FILE *f1)
{
    ...
    while(q)
    {
        if(strcmp(q->attribut->info,"label")==0)remplir_info(in->label,q->valeur->info);
            ...
            in->label->Idelem=-1;
        }else if(strcmp(q->attribut->info,"short")==0){
            in->shor=(int)q->valeur->info[0];
        }
        else if(strcmp(q->attribut->info,"icon")==0)remplir_info(in->icon,q->valeur->info);
        else if(strcmp(q->attribut->info,"signal")==0)remplir_info(in->signal,q->valeur->info);
        q=q->suiv;
    }
    p=in;
    do {
        ...
        if(strcmp(mot,"item")==0)
        {
            ...
            while(q)
            {
                ...
                {
                    ne->shor=(int)q->valeur->info[0];
                }
                else if(strcmp(q->attribut->info,"icon")==0)remplir_info(ne->icon,q->valeur->info);
                else if(strcmp(q->attribut->info,"signal")==0)remplir_info(ne->signal,q->valeur->info);
                q=q->suiv;
            }
            if(k==0)
            {
...
GtkWidget *make_menu_bar(menu_item *menu_item_info,GtkWidget *mn_bar)
{
    ...
    if(menu_item_info->icon->Idelem!=-1)
    {
        ...
        GtkWidget *icon = gtk_image_new_from_icon_name (menu_item_info->icon->info, GTK_ICON_SIZE_MENU);
        GtkWidget *label = gtk_accel_label_new (menu_item_info->label->info);
    ...
    if(menu_item_info->shor != -1)
    {
        gtk_widget_add_accelerator (item, "activate",
                                    accel_group,menu_item_info->shor,GDK_CONTROL_MASK,GTK_ACCEL_VISIBLE);

    }
    if(menu_item_info->signal->Idelem != -1)
    {
        g_signal_connect(item,"activate",G_CALLBACK(on_menuitem_clicked),NULL);
    }
...
{
    GtkWidget *menu;
    GtkPackDirection direction;//gtk_menu_bar_set_pack_direction()  //dir
    menu_item *info;
}menu;
...
typedef struct tli
{
    info *label;//gtk_tool_button_set_label()   //label
    gboolean underline;//gtk_tool_button_set_use_underline()    //unde
    info *icon_name;//gtk_tool_button_set_icon_name ()
    //gtk_tool_button_set_icon_widget ()
    info *tooltip_markup;//gtk_tool_item_set_tooltip_markup ()  //tolmar
    info *tooltip_text;//gtk_tool_item_set_tooltip_text ()  //toltex
    struct tli *suiv;
}tool_item;
...
{
    GtkWidget *toolbar;
    gboolean show_arrow;//gtk_toolbar_set_show_arrow ()     //shoar
    GtkIconSize icon_size;//gtk_toolbar_set_icon_size ()    //icsiz
    GtkToolbarStyle tolbar_style;//gtk_toolbar_set_style () //styl
    tool_item *info;
    //gtk_toolbar_insert ()
}tool_bar;
...
typedef struct cmb
{
    info *text;//gtk_combo_box_text_append_text()   //value
    info *visible;//gtk_combo_box_set_active_id   //visib
    int numvis;//numvis
    struct cmb *suiv;
}combo_box_text;
...
combo_box_text *remplir_com_liste(FILE *f1)
{
    ...
    while (p)
    {
        ...
        else if(strcmp(p->attribut->info,"numvis")==0)
        {
            liste->numvis=atoi(p->valeur->info);
        }
        p=p->suiv;
    }
    fscanf(f1,"%35[^>]>\n",mot);
    ...
    while (strcmp(mot,"/combotext")!=0)
    {
        ...
        while (p)
        {
            if(strcmp(p->attribut->info,"value")==0)remplir_info(ne->text,p->valeur->info);
            else if(strcmp(p->attribut->info,"visib")==0)remplir_info(ne->visible,p->valeur->info);
            else if(strcmp(p->attribut->info,"numvis")==0)
            {
                ne->numvis=atoi(p->valeur->info);
            }
            p=p->suiv;
        }
        ne->suiv=liste;
...
GtkWidget *make_combo_box_text(combo_box_text *info)
{
    ...
    while (p)
    {
        ...
        if(p->visible->Idelem!=-1)
        {
            ...
            gtk_combo_box_set_active(GTK_COMBO_BOX(cmtext),p->numvis);
        }
        p=p->suiv;
    }
...
typedef struct ent
{
    ...
    GtkAlign alignv;//alignv
    int rqs[2];//req
    info *text;//gtk_entry_set_text ()  //text
    gboolean visibilty;//gtk_entry_set_visibility ()    //vis
    gint max_entry;//gtk_entry_set_max_length ()    //mxen
    gboolean frame;//gtk_entry_set_has_frame ()     //fram
    gfloat alignement;//gtk_entry_set_alignment ()  //alig
    info *placeholder;//gtk_entry_set_placeholder_text ()       //plah
    gboolean overwriten;//gtk_entry_set_overwrite_mode ()       //ovew
    info *icon_name;//gtk_entry_set_icon_from_icon_name()       //icnam
    GtkEntryIconPosition icon_position; //icpos
    info *tool_tip_text_icon;//gtk_entry_set_icon_tooltip_text ()   //tooltex
    GtkInputPurpose purpose;//gtk_entry_set_input_purpose ()        //purp
    GtkInputHints inputhints;//gtk_entry_set_input_hints()          //hints
}entry;
...
entry *remplir_entry(FILE *f1)
{
    ...
    while (p)
    {
        ...
        }else if(strcmp(p->attribut->info,"alig")==0)ent->alignement= atof(p->valeur->info);
        else if(strcmp(p->attribut->info,"plah")==0)remplir_info(ent->placeholder,p->valeur->info);
        else if(strcmp(p->attribut->info,"ovew")==0)
        {
        ...
        {
            if(strcmp(p->valeur->info,"GTK_ALIGN_START")==0)ent->alignh=GTK_ALIGN_START;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_CENTER")==0)ent->alignh=GTK_ALIGN_CENTER;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_END")==0)ent->alignh=GTK_ALIGN_END;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_FILL")==0)ent->alignh=GTK_ALIGN_FILL;
        }else if(strcmp(p->attribut->info,"alignv")==0)
        {
            if(strcmp(p->valeur->info,"GTK_ALIGN_START")==0)ent->alignv=GTK_ALIGN_START;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_CENTER")==0)ent->alignv=GTK_ALIGN_CENTER;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_END")==0)ent->alignv=GTK_ALIGN_END;
            else if(strcmp(p->valeur->info,"GTK_ALIGN_FILL")==0)ent->alignv=GTK_ALIGN_FILL;
        }
        p=p->suiv;
    }
    return (entry*)ent;
...
void make_entry(entry *entry_info)
{
    ...
    if(entry_info->rqs[0]!=-1)
    {
        gtk_widget_set_size_request(ent,entry_info->rqs[0],entry_info->rqs[1]);
        gtk_widget_set_halign(ent,entry_info->alignh);
        gtk_widget_set_valign(ent,entry_info->alignv);
    }
...
{
    GtkWidget *serch_bar;
    gboolean search_mod;//gtk_search_bar_set_search_mode ()     //seamod
    gboolean shwo_close_button;//gtk_search_bar_set_show_close_button ()    //shclb
...
typedef struct bx
{
    GtkWidget *box1;
    info *bord;//border
    gboolean homogeneous;//gtk_box_set_homogeneous()    //hom
    GtkOrientation orientation;//orien
    gint spacing;//gtk_box_set_spacing ()   //spac
    GtkBaselinePosition basline_position;//gtk_box_set_baseline_position () //baslp
...
{
    GtkWidget *gri;
    gint rowspacing;//gtk_grid_set_row_spacing ()   //rowspa
    gint colimnspacing;//gtk_grid_set_column_spacing ()     //colspa
    gint row_baseline;//gtk_grid_set_baseline_row()     //spac
    //gtk_grid_attach()
}grid;
...
//    printf("%s\n",mot);
    ...
    }else if(strcmp(mot,"combotext")==0)
    {
        ...
        if(liste)
        {
            sscanf(liste->valeur->info,"%d|%d",&val1,&val2);
        }
        scmt=remplir_com_liste(f1);
        cmt=make_combo_box_text(scmt);
        if(liste)
        {
            gtk_widget_set_size_request(cmt,val1,val2);
            gtk_widget_set_halign(cmt,GTK_ALIGN_START);
            gtk_widget_set_valign(cmt,GTK_ALIGN_START);
        }
